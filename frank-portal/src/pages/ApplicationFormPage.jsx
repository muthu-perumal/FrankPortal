...
export default function ApplicationFormPage() {
  ...
  const isNewApplication = !(transactionId && processId)
  const didApplyDefaultsRef = useRef(false)
  const autoGeneratedDefaultsRef = useRef({})
  ...
  const getAutoGenerateValue = (field) => {
    ...
    if (!id) return null
    if (autoGeneratedDefaultsRef.current[id]) return autoGeneratedDefaultsRef.current[id]
    // Stable per-field value (generated once)
    const v = `AUTO_${id}_${Date.now()}`
    autoGeneratedDefaultsRef.current[id] = v
    return v
  }
  ...
  useEffect(() => {
    // IMPORTANT:
    // Apply default values ONLY for NEW application.
    // If tid/pid exist (editing existing), DO NOT set defaults.
  ...
  }, [allFields.length, isNewApplication])
